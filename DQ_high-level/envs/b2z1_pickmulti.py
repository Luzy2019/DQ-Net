...
from .b2z1_base import B2Z1Base, reindex_all, reindex_feet, LIN_VEL_X_CLIP, ANG_VEL_YAW_CLIP, torch_rand_int
...
class B2Z1PickMulti(B2Z1Base,PredictPoint):
    def __init__(self, cfg_terrain=None, table_height=None,camera_6p_tensor=None, grasp_cv_tensor=None, cube_init_tensor=None, eval=False, *args, **kwargs):
        ...
        self.eval = eval
        self.num_actors = 3
        cprint(f"B2Z1PickMulti is called","cyan")

        B2Z1Base.__init__(self, *args, **kwargs)
    ...
    def _setup_obs_and_action_info(self):
        # b2_z1 now has the gripper DOF like b1z1 (12 legs + 6 arm + 1 gripper),
        # so the roboinfo obs layout matches b1z1.
    ...
    def _extra_env_settings(self):
        self.multi_obj = self.cfg["env"]["asset"]["asset_multi"]  #the cfg here refer to the b2z1_pickmulti.yaml
...
def compute_robot_observations(robot_root_state, table_root_state, cube_root_state, body_pos, 
    ...
    # ee state  13
    if use_roboinfo:
        # b2_z1 URDF is fixed at 19 DOFs (12 legs + 6 arm + 1 gripper), matching the b1z1 layout:
        # dof_pos keeps all 19 DOFs (incl. gripper), dof_vel excludes the gripper (18).
        dof_pos = dof_pos[..., :]
        ...
        # arm_dof_pos / arm_dof_vel: joint1..joint6 (the 6 DOFs before the gripper DOF)
        dof_pos = dof_pos[..., 12:-1]
        dof_vel = dof_vel[..., 12:-1]
    
    base_quat = robot_root_state[:, 3:7]
    arm_base_local = torch.tensor([0.0, 0.0, 0.09], device=robot_root_state.device).repeat(robot_root_state.shape[0], 1) ## b2_z1 arm mount offset
