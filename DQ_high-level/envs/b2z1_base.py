...
class B2Z1Base(RewardVecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, 
        ...
        if not self.floating_base:
            ...
            self.num_gripper_joints = self.num_gripper_dof  # b2_z1: keep gripper DOF (19 total)
    ...
    def _init_tensors(self):
        """Init necessary tensors and buffers for b2z1
        """
        ##### DQ add #####
        ...
        # b2_z1 gripper: gripperMover is the grasp EE link. The arm joints are joint1..joint6
        # and the gripper DOF (jointGripper) is the last DOF; exclude it from the EE Jacobian.
        self.gripper_idx = self.gym.find_actor_rigid_body_index(self.envs[0], self.robot_handles[0], "gripperMover", gymapi.DOMAIN_ENV)
        ...
        if not self.floating_base:
            # b2_z1 low-level policy action_scale: legs=0.35, arm=0.25 (see b2z1_config.control.action_scale)
            low_action_scale = [0.35] * 12 + [0.25] * 6
            self.low_action_scale = torch.tensor(low_action_scale, device=self.device, dtype=torch.float32)
        ...
        # b2_z1: the arm is mounted at the front-center of the body (x_offset=0), goal z at 0.8
        self.ee_goal_center_offset = torch.tensor([0.0, 0.0, 0.8], device=self.device).repeat(self.num_envs, 1)
    ...
    def _set_default_joint_angles_dict(self):
        if self.floating_base:
            ...
            default_joint_angles = { # = target angles [rad] when action = 0.0
                ...
                'joint5': 0.0,
                'joint6': 0.0,
                'jointGripper': -1.57,  # default gripper closed (matches b1z1 z1_jointGripper)
                ####### b2z1 #######
    ...
    def _create_envs(self): # load the b2z1's model, don't relate to the cube's envs
        ...
        # assetFileRobot: "b2z1-col/urdf/b2z1.urdf"
        ...
        for i in range(self.num_envs): 
            ...
            # b2_z1 base height target ~0.48m; spawn slightly above to settle down.
            robot_start_pose.p = gymapi.Vec3(-2.0,2.0,0.60)
    ...
    def _reset_envs(self, env_ids):
        if len(env_ids) > 0:
            ...
            if self.local_step_counter == 0:
                ...
                # b2_z1 initial ee goal (relative to arm base: forward ~0.5m, near table height,
                # inside the b2_z1 reachable workspace pos_l=[0.45,0.82])
                self.curr_ee_goal_cart[:] = torch.tensor([0.5, 0.0, 0.2], device=self.device).repeat(self.num_envs, 1)
    ...
    def _compute_low_level_observations(self):
        ...
        # b2_z1 has the gripper DOF (last), which is excluded from the low-level obs
        # so the low-level policy sees the same 18 DOFs (12 legs + 6 arm) as in training.
        dof_pos_obs = self._dof_pos - self._initial_dof_pos
        dof_vel_obs = self._dof_vel * 0.05
        dof_pos_obs = dof_pos_obs[:, :-self.num_gripper_joints]  # exclude gripper
        dof_vel_obs = dof_vel_obs[:, :-self.num_gripper_joints]

        low_level_obs_buf = torch.cat((self.get_body_orientation(), # dim 2
                                       base_ang_vel, # dim 3
                                       reindex_all(dof_pos_obs), # dim 18
                                       reindex_all(dof_vel_obs), # dim 18
        ...
        if self.mask_arm:
            arm_pos_obs = self._dof_pos - self._initial_dof_pos
            arm_vel_obs = self._dof_vel * 0.05
            arm_pos_obs = arm_pos_obs[:, :-self.num_gripper_joints]
            arm_vel_obs = arm_vel_obs[:, :-self.num_gripper_joints]
            arm_pos_obs = reindex_all(arm_pos_obs)
            arm_pos_obs[:, 12:] = 0
            arm_vel_obs = reindex_all(arm_vel_obs)
            arm_vel_obs[:, 12:] = 0
            low_level_obs_buf = torch.cat((self.get_body_orientation(), # dim 2
                                       base_ang_vel, # dim 3
                                       arm_pos_obs, # dim 18
    ...
    def _draw_ee_goal_target(self):
        ...
        arm_base_local = torch.tensor([0.0, 0.0, 0.09], device=self.device).repeat(self.num_envs, 1)
    ...
    def get_all_pos_targets(self, ee_goal_cart, ee_goal_orn_quat):
        ...
        # b2_z1: arm = joint1..joint6 (the 6 DOFs before the gripper DOF jointGripper)
    ...
    def pre_physics_step(self, action_tensor):
        ...
        self.curr_ee_goal_cart[:, 0] = torch.clip(self.curr_ee_goal_cart[:, 0], 0., 0.85)  # b2_z1 workspace pos_l up to 0.82
    ...
    def set_gripper(self):
        ...
        if self.num_gripper_dof == 0:
            # No gripper DOF: the gripper command is ignored.
            return
        elif self.num_gripper_dof == 1:
    ...
    def update_roboinfo(self):
        # Arm base (b2_z1: arm mount is at the front-center of the body, offset [0.0, 0.0, 0.09])
        base_quat = self._robot_root_states[:, 3:7]
        arm_base_local = torch.tensor([0.0, 0.0, 0.09], device=self.device).repeat(self.num_envs, 1)
